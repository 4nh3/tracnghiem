...
$monhoc = array();
$sql = "SELECT mamon, tenmon FROM cntt";
...
if (isset($_POST['btnSubmit'])) {
    ...
    $OpD = $_POST['OptionD'];
    $ans = $_POST['answer'];
    $sql = "UPDATE `ques_cntt` SET `cauhoi`='$ques', `A`='$OpA', `B`='$OpB', `C`='$OpC', `D`='$OpD', `dapan`='$ans', `mamon`='$mmon' WHERE stt = '$eid'";
    $result = mysqli_query($connect, $sql);
    if ($result) {
        echo "<script>";
...
$sql = "SELECT * FROM `ques_cntt` WHERE stt = '$eid'";
$result = mysqli_query($connect, $sql);
